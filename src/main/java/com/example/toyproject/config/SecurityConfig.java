 ...
 *  - 폼 로그인 페이지 경로 설정 (질문1) 역할과 폼 로그인에 대한 개념 설명
 *  - 비밀번호 Encoder(BCrypt) 등록 (질문2) 등록 의미..?
 **/
...
@Configuration //(질문3) 의미
public class SecurityConfig {
    /*
    * HTTP 통신 보안 규칙 설정
    *  - /login, /auth/signup, 정적리소스는 누구나 접근 가능 (질문4) 정적리소스란?
    *  - 그 외는 인증 필요
    ...
    SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception{
        ...
        http
                ...
                .formLogin(login -> login
                        ...
                        .loginProcessingUrl("/login") //POST /login --> 시큐리티가 인증 처리 해준다.
                        .defaultSuccessUrl("/", true) // 로그인 성공 후 이동 경로 (게시글 작성 페이지...수정)
                        .permitAll()
                )
                // 로그아웃은 기본값 사용 (/logout)
